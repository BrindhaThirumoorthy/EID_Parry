SAP_SERVER = "C:\\Program Files (x86)\\SAP\\FrontEnd\\SAPgui\\saplogon.exe"
Rental_Connection = "EID sandbox"
Rental_Client = "900"
Rental_User = "bcssupport1"
DateContent = {"startDate":"01.12.2024","endDate":"31.12.2024"}
Layout = ["Contracts - Header",]
search_terms = ["Customer Reference","Document Date","Sales Document","Sales Document Type","Valid-To Date","Sold-to Party","Net Value (Item)","Document Currency","Created By","Sold-to Party Name"]